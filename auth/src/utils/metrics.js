...
const httpRequestDuration = new client.Histogram({
  name: `${
    process.env.SERVICE_NAME || "auth_service"
  }_http_request_duration_seconds`,
  help: "Duration of HTTP requests in seconds",
  labelNames: ["service", "instance", "method", "route", "status_code"],
});

// Counters for various auth events
const authCounter = new client.Counter({
  name: `${process.env.SERVICE_NAME || "auth_service"}_auth_events_total`,
...
const rateLimitCounter = new client.Counter({
  name: `${process.env.SERVICE_NAME || "auth_service"}_rate_limit_hits_total`,
